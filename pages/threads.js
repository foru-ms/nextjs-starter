        ...
        <>
            {threads?.slice(0, limit)?.map((thread) => (
                <article key={thread.id}>
                    <Link href={`/thread/${thread.id}`} className="block p-5 transition duration-150 ease-in-out hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-gray-50">
                        <div className="md:flex items-center">
                            <div className="w-full">
                                <div className="md:flex items-center justify-between mt-4 md:mt-0 w-full">
                                    <h2 className="text-gray-800 text-base font-semibold">{thread.title}</h2>
                                    <div className="text-gray-600" aria-label="Bookmark">
                                        <svg
                                ...
                                <div className="md:mt-1 mt-3 flex items-center text-gray-600">
                                    ...
                                    <div className="w-1 h-1 bg-gray-500 rounded-full mx-2" aria-hidden="true" />
                                    <time className="text-gray-600 text-xs" dateTime={thread.createdAt}>
